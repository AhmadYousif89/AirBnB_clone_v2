...
class TestBaseModel(unittest.TestCase):
    ...
    def test_create_with_param(self):
        with patch('sys.stdout', new=StringIO()) as mock_stdout:
            self.console.onecmd(f"create {self.c_name} email=\"xx@gg.cc\"")
        output = mock_stdout.getvalue().strip()
        self.assertIn(f"{self.c_name}.{output}", storage.all().keys())
    ...
    def test_create_with_extra_params(self):
        with patch('sys.stdout', new=StringIO()) as mock_stdout:
            _cmd = f"create {self.c_name} first_name=\"xx\" email=\"xx@gg.cc\""
            self.console.onecmd(_cmd)
        output = mock_stdout.getvalue().strip()
        key = f"{self.c_name}.{output}"
        self.assertIn(key, storage.all().keys())
        obj = storage.all()[key]
        self.assertEqual(obj.__dict__["first_name"], "xx")
        self.assertIn("first_name", obj.__dict__.keys())
        self.assertEqual(obj.__dict__["email"], "xx@gg.cc")
        self.assertIn("email", obj.__dict__.keys())

    def test_create_params_with_underscore_value(self):
        with patch('sys.stdout', new=StringIO()) as mock_stdout:
            _cmd = f"create {self.c_name} first_name=\"xx_cc\""
            self.console.onecmd(_cmd)
        output = mock_stdout.getvalue().strip()
        key = f"{self.c_name}.{output}"
        self.assertIn(key, storage.all().keys())
...
class TestBaseModelDotNotation(unittest.TestCase):
    ...
    def test_show_with_invalid_id(self):
        with patch('sys.stdout', new=StringIO()) as mock_stdout:
            self.console.onecmd(self.console.precmd(f"{self.c_name}.show(1)"))
        output = mock_stdout.getvalue().strip()
        expected = error_messages["no_obj"]
        self.assertEqual(output, expected)
    ...
    def test_destroy_without_id(self):
        with patch('sys.stdout', new=StringIO()) as mock_stdout:
            _cmd = f"{self.c_name}.destroy()"
            self.console.onecmd(self.console.precmd(_cmd))
        output = mock_stdout.getvalue().strip()
        expected = error_messages["no_obj_id"]
        self.assertEqual(output, expected)
